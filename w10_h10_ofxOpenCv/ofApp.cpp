...
void ofApp::update(){
    ...
    if (mVidGrabber.isFrameNew()) {
        ...
        mContourFinder.findContours(mGrayDiff, 100, (320*240), 10, true);
        for(int i = 0; i < mContourFinder.nBlobs;i++){
            particle temp(mContourFinder.blobs[i].boundingRect.getCenter().x+0, mContourFinder.blobs[i].boundingRect.getCenter().y+0);
